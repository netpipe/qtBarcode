...
//        printf("    ECI: %d\n", data->eci);
  ...
  //  std::ostringstream ss;
   // std::copy(data->payload, data->payload+data->payload_len, std::ostream_iterator<int>(ss, " "));
  //  std::cout << ss.str();
   // qDebug() << "test" << data->payload_len;
//    stringstream list;
QString My_String;
    for (int i=0; i< data->payload_len; ++i)
   {
      // list << (int)data->payload[i];
       My_String+=data->payload[i];
   }
//    qDebug() << My_String;
 //   cout << key_string << endl;
//   string key_string = list.str();
//qDebug() << "te" << data->payload[0];
//  // string s=ss.str();
 //  QString test = QString::fromStdString(ss.str());
 // qDebug() << "te" << QString::fromLatin1(test.toLatin1());

//  QString str = QByteArray::fromHex(test.remove(",").toLocal8Bit());
//  qDebug()<<str;
...
static QString dump_info(struct quirc *q)
{
...
QString MainWindow::decodeqr(QString image)
{
	struct quirc *q;
...
//		return -1;
	...
	int status = -1;
    if (check_if_png(image.toLatin1())) {
        status = load_png(q, image.toLatin1());
    } else {
        status = load_jpeg(q,image.toLatin1());
    }
    if (status < 0) {
        quirc_destroy(q);
   //     return -1;
