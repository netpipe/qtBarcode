...
static void PrintUsage2(const char* exePath)
{
...
static std::string FormatClean2(std::string str)
{
...
static std::string ParseFormat2(std::string str)
{
    str = FormatClean2(str);
	for (int i = 0; i < (int)BarcodeFormat::FORMAT_COUNT; ++i) {
		auto standardForm = ToString((BarcodeFormat)i);
        if (str == FormatClean2(standardForm))
			return standardForm;
	}
	return std::string();
}
...
int xzingencode(int argc, char* argv[])
{
	if (argc <= 2) {
        PrintUsage2(argv[0]);
		return 0;
